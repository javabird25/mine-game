...
function UserInfo({userInfo}: { userInfo: IUserInfo }) {
  const hearts = [...Array(maxHealth)].map((_, i) => (
    <img
      src={userInfo.numberLife > i ? fullLife : emptyLife}
      key={i}
      alt=""
      height="23px"
      width="33px"
  ...
  const mana = [...Array(maxMana)].map((_, i) => (
    <img
      src={userInfo.numberMana > i ? fullMana : emptyMana}
      alt=""
      height="30px"
      width="40px"
    />
  ...
  return (
    <div className="userInfo">
      ...
      <div className="userData">
        ...
        <div className="userIconBar">{hearts}</div>
        <div className="userIconBar">{mana}</div>
      </div>
    </div>
  );
}
